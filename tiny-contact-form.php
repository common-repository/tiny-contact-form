...
function TinyContactForm()
{
	...
	add_action('admin_head', array( &$this, 'addAdminStyle'));
	// uninstall function
	if ( function_exists('register_uninstall_hook') )
		register_uninstall_hook(ABSPATH.PLUGINDIR.'/tiny-contact-form/tiny-contact-form.php', array( 'TinyContactForm', 'uninstall')); 
...
function showForm( $params = '' )
{
	...
	if ( empty($result) || (!empty($result) && $result != $this->o['msg_ok'] && !$this->o['hideform']) )
	{
		// subject from form
...
{	
	...
	{
		$to = wp_strip_all_tags($_POST['tcf_to_email']);
		if ( empty($to) )
			$to = get_option('admin_email');
		$msg_ok = wp_strip_all_tags($_POST['tcf_msg_ok']);
		if ( empty($msg_ok) )
			$msg_ok = "Thank you! Your message was sent successfully.";
		$msg_err = wp_strip_all_tags($_POST['tcf_msg_err']);
		if ( empty($msg_err) )
		...
		$cc = ( isset($_POST['tcf_cc']) ) ? 1 : 0;
		
		$this->o = array(
			'to_email'			=> $to,
			'from_email'		=> wp_strip_all_tags($_POST['tcf_from_email']),
			'css'				=> wp_strip_all_tags($_POST['tcf_css']),
			'msg_ok'			=> $msg_ok,
			'msg_err'			=> $msg_err,
			'submit'			=> wp_strip_all_tags($_POST['tcf_submit']),
			'captcha'			=> $captcha,
			'captcha_label'		=> wp_strip_all_tags($_POST['tcf_captcha_label']),
			'captcha2'			=> $captcha2,
			'captcha2_question'	=> wp_strip_all_tags($_POST['tcf_captcha2_question']),
			'captcha2_answer'	=> wp_strip_all_tags($_POST['tcf_captcha2_answer']),
			'subpre'			=> wp_strip_all_tags($_POST['tcf_subpre']),
			'field_1'			=> wp_strip_all_tags($_POST['tcf_field_1']),
			'field_2'			=> wp_strip_all_tags($_POST['tcf_field_2']),
			'field_3'			=> wp_strip_all_tags($_POST['tcf_field_3']),
			'field_4'			=> wp_strip_all_tags($_POST['tcf_field_4']),
			'field_5'			=> wp_strip_all_tags($_POST['tcf_field_5']),
			'hideform'			=> $hideform,
			'cc'				=> $cc
			);
	...
	<div class="postbox">
		...
		<fieldset>
		<legend><?php _e('Form', 'tcf-lang'); ?></legend>
		...
		<tr>
			<th><?php _e('CC', 'tcf-lang')?>:</th>
			<td><label for="tcf_cc"><input name="tcf_cc" id="tcf_cc" type="checkbox" <?php if($this->o['cc']==1) echo 'checked="checked"' ?> /> <?php _e('It is possible to send a copy of the message to sender.', 'tcf-lang'); ?></label></td>
		</tr>
    	<tr>
		...
		<fieldset>
		<legend><?php _e('Captcha', 'tcf-lang'); ?></legend>
		...
		<fieldset>
		<legend><?php _e('Style', 'tcf-lang'); ?></legend>
...
{
	global $wp_version;
	$menutitle = '<img src="'.$this->getResource('tcf_menu.png').'" alt="" /> Tiny Contact Form';
	add_options_page('Tiny Contact Form', $menutitle, 'manage_options', 'tiny-contact-form', array( &$this, 'optionsPage'));
}
...
/**
 * adds admin style for settingspage
 */
function addAdminStyle()
{
	if ($_SERVER['QUERY_STRING'] && strpos($_SERVER['QUERY_STRING'], 'tiny-contact-form') !== false) :
	?>
	<style type="text/css">
	.tcf-settings fieldset {margin:10px 5px 10px;padding:0 10px 0px;border:1px solid #dfdfdf;border-radius:3px;background:#fafafa;}
	.tcf-settings legend {background:#eee;border:1px solid #dfdfdf;border-radius:3px;margin:0 10px;padding:1px 7px 2px;text-shadow:0 1px 0 #fff;font-family:Georgia,"Times New Roman","Bitstream Charter",Times,serif;font-size:15px;}
	</style>
	<?php
	endif;
}
...
//  var_dump($this->fields);
		...
		{ 
			...
			$field_value = (!empty($instance[$f])) ? esc_attr( $instance[$f] ) : '';
